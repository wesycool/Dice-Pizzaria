...
document.querySelectorAll('#selectionBtn').forEach( button => {
    button.addEventListener('click',async () => {
        const products = await fetch('/staff-portal/api/table/products').then(response => response.json()).then(data => {
            return data
        })
...
document.querySelector('#statusBtn').addEventListener('click', async () => {
    ...
    if (!document.querySelector('#hiddenClientId').value) {
        const clientInfo = await fetch(`/staff-portal/api/table/client/email/${email}`).then(res => res.json()).then(data => {
            return data
        } )
        ...
        if(clientInfo.length != 0){
            ...
            if (clientInfo[0].first_name != firstname) {
                fetch(`/staff-portal/api/client/first_name/${firstname}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].last_name != lastName) {
                fetch(`/staff-portal/api/client/last_name/${lastName}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].address != address) {
                fetch(`/staff-portal/api/client/address/${address}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].address2 != address2) {
                fetch(`/staff-portal/api/client/address2/${address2}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].country != country) {
                fetch(`/staff-portal/api/client/country/${country}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].province != province) {
                fetch(`/staff-portal/api/client/province/${province}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].city != city) {
                fetch(`/staff-portal/api/client/city/${city}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].postal_code != postal) {
                fetch(`/staff-portal/api/client/postal_code/${postal}/${getID}`,{method:'PUT'})
            }
            if (clientInfo[0].phone != phone) {
                fetch(`/staff-portal/api/client/phone/${phone}/${getID}`,{method:'PUT'})
            }
